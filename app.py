...
def markAttendance(name):
    ...
    df = pd.read_csv("Database/Student_database.csv", index_col=0)
    try:
        message = df.loc[name,"Message"]
    except:
        message = "Nil Message"
    return message
    ...
    while run_attendee:
        ...
        for encodeFace,faceLoc in zip(encodesCurFrame,facesCurFrame):
            ...
            if np.amin(faceDis)>0.6:
                ...
                cv2.rectangle(img,(x1,y2-70),(x2,y2),(0,255,0),2)
                cv2.putText(img,message,(x1+6,y2-41),cv2.FONT_HERSHEY_COMPLEX,1,(255,255,255),2)
